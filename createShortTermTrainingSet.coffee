...
main = ->
  length = 128
  fft = do (fft = new FFT(length, 20))-> (arr)->
    _arr = arr.slice()
    fft.forward(_arr)
  ...
  convoluted_matrix = []
  for v,i in casted_matrix by length/4|0
    convoluted_matrix.push casted_matrix.slice(i,i+128)
  convoluted_matrix = convoluted_matrix.filter (a)-> a.length is length
  transposed_matrix = convoluted_matrix.map (_arr)->
    cols = ([] for i in [1.._arr[0].length])
    _arr.forEach (row)->
      row.forEach (data, i)->
        cols[i].push data
    cols
